...
def lucas_kanade_step(I1: np.ndarray,
    ...
    I2_f = I2.astype(np.float64)
    Ix = signal.convolve2d(I2_f, X_DERIVATIVE_FILTER, boundary='symm', mode='same')
    Iy = signal.convolve2d(I2_f, Y_DERIVATIVE_FILTER, boundary='symm', mode='same')
    # Temporal gradient between frames
    It = I2_f - I1.astype(np.float64)

    # Pre-compute all window sums with a ones-kernel so the inner loop only
    # does scalar lookups instead of per-pixel array slicing + matrix ops.
    ones = np.ones((window_size, window_size))
    Ixx_map = signal.convolve2d(Ix * Ix, ones, boundary='symm', mode='same')
    Iyy_map = signal.convolve2d(Iy * Iy, ones, boundary='symm', mode='same')
    Ixy_map = signal.convolve2d(Ix * Iy, ones, boundary='symm', mode='same')
    Ixt_map = signal.convolve2d(Ix * It, ones, boundary='symm', mode='same')
    Iyt_map = signal.convolve2d(Iy * It, ones, boundary='symm', mode='same')
    ...
    valid = np.zeros(I1.shape, dtype=bool)
    valid[half_window:height - half_window,
          half_window:width - half_window] = True

    det_map = Ixx_map * Iyy_map - Ixy_map ** 2
    valid &= np.abs(det_map) > 1e-6
    du[valid] = (Iyy_map[valid] * (-Ixt_map[valid]) -
                 Ixy_map[valid] * (-Iyt_map[valid])) / det_map[valid]
    dv[valid] = (Ixx_map[valid] * (-Iyt_map[valid]) -
                 Ixy_map[valid] * (-Ixt_map[valid])) / det_map[valid]
...
def faster_lucas_kanade_step(I1: np.ndarray,
    ...
    corner_mask = harris > 0.005 * harris.max()
    ...
    # Pre-compute window sums for all pixels at once
    ones = np.ones((window_size, window_size))
    Ixx_map = signal.convolve2d(Ix * Ix, ones, boundary='symm', mode='same')
    Iyy_map = signal.convolve2d(Iy * Iy, ones, boundary='symm', mode='same')
    Ixy_map = signal.convolve2d(Ix * Iy, ones, boundary='symm', mode='same')
    ...
    half_w = window_size // 2
    border_mask = np.zeros(I1.shape, dtype=bool)
    border_mask[half_w:-half_w, half_w:-half_w] = True

    # Apply LK only at valid corner pixels (vectorized, no Python loop)
    det_map = Ixx_map * Iyy_map - Ixy_map ** 2
    valid = corner_mask & border_mask & (np.abs(det_map) > 1e-6)
    du[valid] = (Iyy_map[valid] * (-Ixt_map[valid]) - Ixy_map[valid] * (-Iyt_map[valid])) / det_map[valid]
    dv[valid] = (Ixx_map[valid] * (-Iyt_map[valid]) - Ixy_map[valid] * (-Ixt_map[valid])) / det_map[valid]
...
def faster_lucas_kanade_optical_flow(
    ...
    for level in range(num_levels, -1, -1):
        ...
        # At the coarsest level u and v are all-zeros, so warp is identity — skip it
        if level == num_levels:
            I2_warp = I2_level.astype(np.float64)
        else:
            I2_warp = warp_image(I2_level, u, v)
...
def lucas_kanade_faster_video_stabilization(
    ...
    for _ in tqdm(range(1, frame_count), desc="Stabilizing Video FASTER"):
        ret, frame = cap.read()
        if not ret:
            break
        ...
        valid_du = du[half_w:-half_w, half_w:-half_w]
        valid_dv = dv[half_w:-half_w, half_w:-half_w]
        corner_flow = np.abs(valid_du) + np.abs(valid_dv) > 1e-10
        if corner_flow.any():
            du_mean = float(np.median(valid_du[corner_flow]))
            dv_mean = float(np.median(valid_dv[corner_flow]))
        else:
            du_mean = 0.0
            dv_mean = 0.0
...
def lucas_kanade_faster_video_stabilization_fix_effects(
    ...
    for _ in tqdm(range(1, frame_count), desc="Stabilizing Video FASTER + FIXED BORDERS"):
        ret, frame = cap.read()
        if not ret:
            break
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
